...
parser.add_argument('--e_loss_lambda', required=False, default=1.0, type=float)
...
comp_gid_class_weights = torch.Tensor(compute_class_weight('balanced', classes=[0, 1, 2, 3], y=all_train_comp_labels)).to(device)
...
losses = ['row_col_gid', 'edge', 'constraint']
coeffs = [1.0, args.e_loss_lambda, args.c_loss_lambda]
...
def train_model(epoch):
    ...
    for batch_data in tepoch:
        ...
        (comp_gid_logits, comp_gid_labels), (edge_logits, edge_labels), (batch_loss[losses[2]], ), = model(batch_data)
...
for epoch in range(num_epochs):
    ...
    val_stats = eval_model('val_non_scc')
    print('Val Stats\n', val_stats)
    test_stats = eval_model('test_non_scc')
    print('Test Stats\n', test_stats)
    print()
...
for s in splits[1:]:
    ...
    res[s]['composition_metrics'], res[s]['type_labels'] = eval_model(s, debug=True)
