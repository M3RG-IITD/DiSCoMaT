...
class VerticalAttentionTableBert(VanillaTableBert):
    ...
    def __init__(
        ...
        **kwargs
        ...
        print(f"config.cache_dir {self.config.cache_dir}")
        if config.base_model_name == 'm3rg-iitd/matscibert':
            matscibert_config = AutoConfig.from_pretrained(config.base_model_name, cache_dir=self.config.cache_dir)
            self._bert_model = AutoModel.from_pretrained(config.base_model_name, config=matscibert_config, cache_dir=self.config.cache_dir)
        else:
            self._bert_model = BertForMaskedLM.from_pretrained(config.base_model_name, cache_dir=self.config.cache_dir)
    ...
    def forward(
        ...
        **kwargs
        ...
        bert_output = bert_output[0].view(batch_size, max_row_num, sequence_len, -1)
    ...
    def forward_for_discomat(
        ...
        **kwargs
        ...
        del flattened_input_ids, flattened_segment_ids, flattened_sequence_mask
        ...
        del context_token_mask, table_encoding, table_mask 
        ...
        if masked_column_token_labels is not None:
            ...
            if self.config.predict_cell_tokens:
                ...
                del masked_cell_representation, masked_cell_token_position_embedding, cell_token_scores
            ...
            del column_token_span_representation

            masked_context_token_loss = loss_fct(context_token_scores.view(-1, self.config.vocab_size), masked_context_token_labels.view(-1))
            masked_context_token_num = masked_context_token_labels.ne(-1).sum().item()

            del context_token_scores

            masked_column_token_loss = loss_fct(column_token_scores.view(-1, self.config.vocab_size), masked_column_token_labels.view(-1))
            masked_column_token_num = masked_column_token_labels.ne(-1).sum().item()

            del column_token_scores
            ...
            return context_encoding, schema_encoding, final_table_encoding
    ...
    def vertical_transform(self, context_encoding, context_token_mask, table_encoding, table_mask):
        ...
        del sequence_mask, attention_mask, bert_output, hidden_states, vertical_layer_outputs
        ...
        del last_context_encoding, table_row_nums
        torch.cuda.empty_cache()
    ...
    # noinspection PyUnboundLocalVariable
    def to_tensor_dict_for_discomat(
        self,
        contexts: List[List[str]],
        tables: List[DiscomatTable],
        table_specific_tensors=True
    ):
        examples = []
        for e_id, (context, table) in enumerate(zip(contexts, tables)):
            instance = self.input_formatter.get_input_for_discomat(context, table)
    ...
    def encode_for_discomat(
        ...
        tensor_dict, instances = self.to_tensor_dict_for_discomat(contexts, tables)
        tensor_dict = {
            k: v.to(self.device) if torch.is_tensor(v) else v
            for k, v in tensor_dict.items()
        }

        context_encoding, schema_encoding, final_table_encoding = self.forward_for_discomat(**tensor_dict)
        ...
        return context_encoding, schema_encoding, final_table_encoding, info
