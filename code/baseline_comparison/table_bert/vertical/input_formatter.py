...
from table_bert.table import Column, Table, DiscomatTable
...
class VerticalAttentionTableBertInputFormatter(VanillaTableBertInputFormatter):
    ...
    def get_input_for_discomat(self, context: List[str], table: DiscomatTable):
        row_instances = []
        ...
        for row_data in table_data:
            ...
            row_instance = self.get_row_input_for_discomat(context, row_data)
            row_instances.append(row_instance)
    ...
    def get_row_input(self, context: List[str], header: List[Column], row_data: List[Any]):
        ...
        for col_id, column in enumerate(header):
            if col_id < len(row_instance['column_spans']):
                col_start, col_end = row_instance['column_spans'][col_id][self.column_span_method] # seld.column_span_method??
    ...
    def get_row_input_for_discomat(self, context: List[str], row_data: List[Any]):
        row_instance = super(VerticalAttentionTableBertInputFormatter, self).get_row_input_for_discomat(
        ...
        for col_id, value_tokens in enumerate(row_data):
            ...
            if col_id < len(row_instance['column_spans']):
                ...
                col_start, col_end = row_instance['column_spans'][col_id][self.column_span_method] # self.column_span_method??
