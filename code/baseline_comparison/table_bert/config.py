...
class TableBertConfig(SimpleNamespace):
    def __init__(
        ...
        cell_input_template: str = 'type | value', # changing default template for discomat which does not use column headers
        ...
        **kwargs
        ...
        if not hasattr(self, 'vocab_size_or_config_json_file'):
            ...
            if bert_config:
                # set attributes only if bert config has been defined for the base_model
                for k, v in vars(bert_config).items():
                    setattr(self, k, v)
            elif self.base_model_name == 'm3rg-iitd/matscibert':
                print(f"cache_dir in table_bert_config init {cache_dir}")
                bert_config = AutoConfig.from_pretrained(self.base_model_name, cache_dir=cache_dir)
                for k, v in vars(bert_config).items():
                    setattr(self, k, v)
