...
class TableBertModel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._bert_model = bert_model # set to None, is overwritten in VanillaTableBert and VerticalAttentionTableBert

        if config.base_model_name == 'm3rg-iitd/matscibert':
            self.tokenizer = AutoTokenizer.from_pretrained(config.base_model_name, cache_dir=config.cache_dir, model_max_length=512)

        else:
            
            # print("self._bert_model", self._bert_model) # is None
            # print("config.base_model_name", config.base_model_name) # bert-base-uncased
    ...
    def bert(self) -> BertModel:
        ...
        if hasattr(self._bert_model, 'bert'):
            # print("self._bert_model", self._bert_model)
            # print("hasattr(self._bert_model, 'bert')", hasattr(self._bert_model, 'bert'))
    ...
    def from_pretrained(
        ...
        **kwargs
        ...
        if model_name_or_path in {'bert-base-uncased', 'bert-large-uncased', 'm3rg-iitd/matscibert'}:
            # config = TableBertConfig(base_model_name=model_name_or_path)
            cache_dir = kwargs.pop('cache_dir', None)
            config = VerticalAttentionTableBertConfig(base_model_name=model_name_or_path, cache_dir=cache_dir) # cache_dir is passed in kwargs
            ...
            # model = VanillaTableBert(config)
            model = VerticalAttentionTableBert(config)
