...
class VanillaTableBertInputFormatter(TableBertBertInputFormatter):
    ...
    def get_cell_input_for_discomat(
        self,
        # column: Column,
        cell_value: List[str],
        token_offset: int
        ...
        # print(f"self.config.cell_input_template {self.config.cell_input_template}")
        ...
        for token in self.config.cell_input_template:
            ...
            if token == 'value':
                ...
                span_map['type'] = (start_token_abs_position,
                                    start_token_abs_position + 1)
                # infer type of cell: text, alphanumeric or numeric
                str_cell_value = "".join(cell_value)
                if str_cell_value.replace('.','',1).isnumeric():
                    cell_type = 'numeric'
                ...
                elif all(x.isalpha() or x.isspace() or x in string.punctuation for x in str_cell_value):
                    # empty cells will get type=text
                    cell_type = 'text'
                else:
                    cell_type = 'alphanumeric'
                # print(f"cell value {str_cell_value}, inferrred type {cell_type}")
                input.append(cell_type)
            else:
                # print(f"SEE WARNING: going into other token type than value or type. token type={token}.")
                span_map.setdefault('other_tokens', []).append(start_token_abs_position)
                input.append(token)
    ...
    def get_row_input(self, context: List[str], header: List[Column], row_data: List[Any], trim_long_table=False):
        ...
        for col_id, column in enumerate(header):
            ...
            # print("column_input_tokens", column_input_tokens)
    ...
    def get_row_input_for_discomat(self, context: List[str], row_data: List[Any], trim_long_table=False):
        if self.config.context_first:
            table_tokens_start_idx = len(context) + 2  # account for [CLS] and [SEP]
            # account for [CLS] and [SEP], and the ending [SEP]
            max_table_token_length = MAX_BERT_INPUT_LENGTH - len(context) - 2 - 1
        ...
        for col_id, value_tokens in enumerate(row_data):
            # if len(value_tokens) > self.config.max_cell_len:
                # print(f"SEE WARNING: truncating row: from len(value_tokens)={len(value_tokens)} to self.config.max_cell_len={self.config.max_cell_len}.")
            truncated_value_tokens = value_tokens[:self.config.max_cell_len]

            column_input_tokens, token_span_map = self.get_cell_input_for_discomat(
                # column,
                truncated_value_tokens,
                token_offset=column_start_idx
            )
            ...
            if trim_long_table:
                if len(row_input_tokens) + len(column_input_tokens) > max_table_token_length:
                    ...
                    for key in token_span_map:
                        # if key in {'column_name', 'type', 'value', 'whole_span'}:
                        if key in {'type', 'value', 'whole_span'}:
                            span_start_idx, span_end_idx = token_span_map[key]
                            if span_start_idx < end_index < span_end_idx:
                                token_span_map[key] = (span_start_idx, end_index)
                            elif end_index < span_start_idx:
                            ...
                            if not new_positions:
                                keys_to_delete.append(key)
                            del old_positions, new_positions
        ...
        if self.config.context_first:
            ...
            # print("SEE WARNING: self.config.context_first is False")
            sequence = ['[CLS]'] + row_input_tokens + ['[SEP]'] + context + ['[SEP]']
            # segment_ids = [0] * (len(row_input_tokens) + 2) + [1] * (len(context) + 1)
            segment_a_length = len(row_input_tokens) + 2
            context_span = (len(row_input_tokens) + 1, len(row_input_tokens) + 1 + 1 + len(context) + 1)
        ...
        # print(f"linearized row tokens: {sequence}")
