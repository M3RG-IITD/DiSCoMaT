...
class TabertAdaptedBaselineModel(nn.Module):
    ...
    def get_edges(r, c):
        ...
        row_edges = TabertAdaptedBaselineModel.get_all_pair(np.arange(c))
        for i in range(r):
            edges = np.concatenate((edges, row_edges + i * c), axis=0)
        col_edges = TabertAdaptedBaselineModel.get_all_pair(np.arange(0, r * c, c))
        for i in range(c):
    ...
    def __init__(self, args: dict):
        super(TabertAdaptedBaselineModel, self).__init__()
        self.args = args

        self.vertical_attention_table_bert_model = VerticalAttentionTableBert.from_pretrained('m3rg-iitd/matscibert', cache_dir=args['cache_dir'])
